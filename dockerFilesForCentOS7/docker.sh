...
sudo docker run --restart=always -e container_name=${container_name} -it --tmpfs /run -v /sys/fs/cgroup:/sys/fs/cgroup:ro -d -p ${https_port}:443 --privileged --name $container_name -v $container_name:/$container_name bahmni/bahmni_centos:${container_name} /sbin/init
